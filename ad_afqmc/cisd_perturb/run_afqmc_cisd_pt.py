...
init_ept, init_eog = trial._calc_energy_pt_restricted(
...
if rank == 0:
    ...
    print(f"# {0:5d} \t {init_ept:.7f} \t {init_eog:.7f} \t {init_time:.2f}")
comm.Barrier()
...
for n in range(1, neql + 1):
    ...
    if rank == 0:
        print(
            f"# {n:5d} \t {blk_ept[0]:.7f} \t {blk_eog[0]:.7f} \t {time.time() - init:.2f} ",
            flush=True,
        )
    comm.Barrier()
...
for n in range(sampler.n_blocks):
    ...
    if rank == 0:
        ...
        blk_ept = np.sum(gather_wt * gather_eog) / blk_wt
    comm.Barrier()
