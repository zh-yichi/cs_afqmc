...
init_ept = cisd_pt._cisd_walker_energy_pt(
...
if rank == 0:
    ...
    print(f"# {0:5d} \t {init_ept:.7f} \t {init_time:.2f}")
comm.Barrier()

for n in range(1, neql + 1):
    prop_data, (blk_wt, blk_ept) = sample_pt.propagate_phaseless(
    ...
    if rank == 0:
        print(
            f"# {n:5d} \t {blk_ept[0]:.7f} \t {time.time() - init:.2f} ",
            flush=True,
        )
    comm.Barrier()
...
for n in range(sampler.n_blocks):
    ...
    if n % (max(sampler.n_blocks // 10, 1)) == 0:
        ...
        if rank == 0:
                ...
                print(f"  {n:4d} \t \t {ept} \t {ept_err} \t {time.time() - init:.2f}")
        comm.Barrier()
