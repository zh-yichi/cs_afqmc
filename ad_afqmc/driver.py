...
def LNOafqmc(
    ...
    for n in range(1, neql + 1):
        block_energy_n, prop_data, block_orbE_n = sampler_eq.propagate_phaselessLNO(
    ...
    if rank == 0:
        global_block_weights = np.zeros(size * sampler.n_blocks)
        global_block_energies = np.zeros(size * sampler.n_blocks)
        global_block_orbE = np.zeros(size * sampler.n_blocks)
        global_block_observables = np.zeros(size * sampler.n_blocks)
        if options["ad_mode"] == "reverse":
            global_block_rdm1s = np.zeros(
                (size * sampler.n_blocks, *(ham_data["h1"].shape))
    ...
    for n in range(sampler.n_blocks):
        if options["ad_mode"] == "forward":
            ...
            block_energy_n, prop_data, block_orbE_n = sampler.propagate_phaselessLNO(
                ham, ham_data, propagator, prop_data, trial, wave_data, orbE = orbE
            )
            block_observable_n = 0.0
        ...
        if rank == 0:
            global_block_weights[n * size : (n + 1) * size] = gather_weights
            global_block_energies[n * size : (n + 1) * size] = gather_energies
            global_block_orbE[n * size : (n + 1) * size] = gather_orbE
            global_block_observables[n * size : (n + 1) * size] = gather_observables
            if options["ad_mode"] == "reverse":
                global_block_rdm1s[n * size : (n + 1) * size] = gather_rdm1s
            elif options["ad_mode"] == "2rdm":
            ...
            block_orbE_n = np.sum(gather_weights * gather_orbE) / np.sum(
                gather_weights
            )
        ...
        if n % (max(sampler.n_blocks // 10, 1)) == 0:
            ...
            if rank == 0:
                ...
                orbE_afqmc, orbE_error = stat_utils.blocking_analysis(
                    global_block_weights[: (n + 1) * size],
                ...
                if energy_error is not None and orbE_error is not None:
                    if options["ad_mode"] is None:
                        print(
                            f" {n:5d}      {e_afqmc:.9e}        {energy_error:.9e}        {orbE_afqmc:.9e}        {orbE_error:.9e}        {time.time() - init:.2e} ",
                            flush=True,
                        )
                    else:
                        print(
                    ...
                    if options["ad_mode"] is None:
                        print(
                            f" {n:5d}      {e_afqmc:.9e}                -              {orbE_afqmc:.9e}        -       {time.time() - init:.2e} ",
                            flush=True,
                        )
                    else:
                        print(
    ...
    if rank == 0:
        ...
        if e_err_afqmc is not None:
            ...
            print(f"AFQMC energy: {e_afqmc}\n", flush=True)
            e_err_afqmc = 0.0
        if(err_orbE is not None):
            sig_dec = int(abs(np.floor(np.log10(err_orbE))))
            sig_err = np.around(np.round(err_orbE * 10**sig_dec) * 10**(-sig_dec), sig_dec)
            sig_e = np.around(orbE_afqmc, sig_dec)
            print(f'orbE energy: {sig_e:.{sig_dec}f} +/- {sig_err:.{sig_dec}f}\n',flush = True)
        else:
            print(f'orbE energy: {orbE_afqmc} +/- None\n', flush=True)
