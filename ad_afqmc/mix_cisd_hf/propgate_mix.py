...
def _block_scan(
    ...
    en_samples = jnp.where(
        jnp.abs(en_samples - prop_data["e_estimate"]) > jnp.sqrt(2.0 / prop.dt),
        prop_data["e_estimate"],
        en_samples,
    )

    blk_wt = jnp.sum(prop_data["weights"])
    blk_en = jnp.sum(en_samples * prop_data["weights"]) / blk_wt
...
def propagate_phaseless(
    ...
    return prop_data, (wt, en)


def run_mixed_prop(options,nproc=None,option_file='options.bin'):
    ...
    script = f"{dir_path}/run_afqmc_mix_prop.py"
    
    os.system(
        f"export OMP_NUM_THREADS=1; export MKL_NUM_THREADS=1;"
        f"{mpi_prefix} python {script} {gpu_flag} |tee afqmc_mix_prop.out"
    )
