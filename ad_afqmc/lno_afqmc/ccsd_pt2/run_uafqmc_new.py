...
if rank == 0:
    ...
    print("#   Iter \t Energy_HF \t time")
    print(f"  {0:5d} \t {jnp.real(e0):.6f} \t {time.time() - init_time:.2f}")
comm.Barrier()
...
for n in range(1,options["n_eql"]+1):
    prop_data, (wt, e0) = sampler_eq.propagate_phaseless(prop_data, ham_data, prop, trial, wave_data)
    ...
    comm.Barrier()
    if rank == 0:
        print(f"  {n:5d} \t {e0:.6f} \t {time.time() - init_time:.2f} ")
    comm.Barrier()
...
if rank == 0:
    ...
    print(f"# AFQMC/CCSD_PT2 Orbital Ept (covariance): {eorb_pt:.6f} +/- {eorb_pt_err:.6f}")
