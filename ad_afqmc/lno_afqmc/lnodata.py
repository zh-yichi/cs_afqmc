...
def lno_data(nfrag, lno_thresh, outfile='lno_result.out'):
    ...
    for n, i in enumerate(frg_list):
        with open(f"lnoafqmc.out{i+1}", "r") as rf:
            for line in rf:
                ...
                if "energy (Raw)" in line:
                    eorb_pt2raw[n] = float(line.split()[-3])
                    eorb_pt2raw_err[n] = float(line.split()[-1])
    ...
    with open(outfile, 'w') as out_file:
        print('# frag \t eorb_afqmc/ccsd_pt2(raw) \t eorb_afqmc/ccsd_pt2(mah) \t eorb_afqmc/ccsd_pt2(dir) \t nelec \t norb \t \t time', file=out_file)
        for n, i in enumerate(frg_list):
