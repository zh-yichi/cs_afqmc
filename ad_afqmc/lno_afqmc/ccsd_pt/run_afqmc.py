...
e0 = jnp.array(jnp.sum(e0)/ prop.n_walkers)
eorb_pt = eorb + teorb - torb * (e0-h0)
...
if rank == 0:
    ...
    print("#   Iter \t Energy_hf \t <H-E0>_orb \t <TH>_orb \t <T>_orb \t Ept_orb \t time")
    print(f"  {0:5d} \t {e0:.6f} \t {eorb:.6f} \t {teorb:.6f} \t" 
...
for n in range(1,options["n_eql"]+1):
    prop_data, (wt, eorb, teorb, torb, e0) = \
...
if rank == 0:
    print("#\n Sampling sweeps:")
    print("#  Iter   Energy_hf   error   <H-E0>_orb    error "
...
for n in range(sampler.n_blocks):
    ...
    if n % (max(sampler.n_blocks // 10, 1)) == 0 and n > 0:
        ...
        if rank == 0:
            ...
            eorb_pt = eorb + teorb - torb * (e0 - h0)
            # (p_ept/p_eorb, p_ept/p_teorb, p_ept/p_torb, p_ept/p_e0)
            dE = np.array([1,1,-e0+h0,-torb])
...
if rank == 0:
    ...
    eorb_pt = eorb + teorb - torb * (e0 - h0)
    dE = np.array([1,1,-e0+h0,-torb])
    cov = np.cov([glb_eorb,glb_teorb,glb_torb,glb_e0])
