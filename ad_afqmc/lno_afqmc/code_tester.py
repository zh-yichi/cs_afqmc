...
#### test norhf #######
@jax.jit
def _frg_norhf_eorb_test(walker,ham_data,wave_data):
    m = jnp.dot(wave_data["prjlo"].T,wave_data["prjlo"])
    trial_mo = wave_data['mo_coeff']
    rot_h1,rot_chol = ham_data['rot_h1'],ham_data['rot_chol']
    nocc = rot_h1.shape[0]
    ...
    e_exc = -jnp.einsum('gij,gjk,ik->',f,f,m)
    ene2 = e_col+e_exc
    return jnp.real(ene1+ene2), jnp.real(ene1), jnp.real(ene2)

@jax.jit
def frg_norhf_eorb_test(walkers,ham_data,wave_data):
    hf_orb_en,ene1,ene2 = vmap(_frg_norhf_eorb_test, in_axes=(0, None, None))(
        walkers, ham_data, wave_data)
    return hf_orb_en,ene1,ene2
