...
if rank == 0:
    if isinstance(trial, wavefunctions.rhf):
        if which_rhf == 1:
            ...
            orb_en = code_tester._frg_norhf_eorb(
...
for n in range(options["n_eql"]):
    prop_data, (blk_wt,blk_en,blk_orb_en) \
                = code_tester.propagate_phaseless_orb(
                    ham_data,prop,prop_data,trial,wave_data,sampler,which_rhf)
