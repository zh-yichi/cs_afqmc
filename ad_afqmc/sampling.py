...
class sampler:
    ...
    @partial(jit, static_argnums=(0,4,5))
    def _block_scan_LNO(self, prop_data: dict, _x, ham_data:dict, propagator:propagator, trial:wave_function, wave_data:dict,orbE:int):
        prop_data['key'], subkey = random.split(prop_data['key'])
        fields = random.normal(subkey, shape=(self.n_prop_steps, propagator.n_walkers, ham_data['chol'].shape[0]))
        _step_scan_wrapper = lambda x, y: self._step_scan(x, y, ham_data, propagator, trial, wave_data)
        prop_data, _ = lax.scan(_step_scan_wrapper, prop_data, fields)
        ...
        orbE_samples = jnp.real(trial.calc_orbenergy(prop_data['walkers'], ham_data, wave_data,orbE))
    ...
    def propagate_phaselessLNO(self,ham, ham_data, propagator, prop_data, trial, wave_data,orbE):
        def _sr_block_scan_wrapper(x, y): return self._sr_block_scan_LNO(x, y, ham_data, propagator, trial, wave_data,orbE=orbE)
