...
#     print(f" {n*block_time:.2f}  {ehf:.6f}  {ecc:.6f}  {time.time() - init_time:.2f} ")
...
for n in range(sampler.n_blocks):
    ...
    if (n+1) % (max(sampler.n_blocks // 10, 1)) == 0 and n > 0:
        ...
        print(f" {n+1:4d}  {ehf_avg:.6f}  {ehf_err:.6f}  {ecc_avg:.6f}  {ecc_err:.6f}  {time.time() - init_time:.2f}")

print('# Post Propagation')

ecc_raw = np.real(whf_num / whf_den)
ecc_err_raw = sampler.blk_average(whf_sp, num_sp, den_sp, max_size=10)

find_err = False
for i, err in enumerate(ecc_err_raw):
    if np.abs((err - ecc_err_raw[i-1]) / err) < 0.04:
        find_err = True
