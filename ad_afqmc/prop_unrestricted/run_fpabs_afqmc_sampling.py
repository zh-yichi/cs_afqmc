...
comm.Barrier()
if rank == 0:
    glb_blk_w = np.zeros((sampler.n_eql_blocks, sampler.n_trj), dtype="float64")
    glb_blk_e = np.zeros((sampler.n_eql_blocks, sampler.n_trj), dtype="float64")
comm.Barrier()
...
for n in range(sampler.n_trj):
    ...
    blk_w = np.array([blk_w], dtype="float64")
